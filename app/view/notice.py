...
def revealed_user_qr_code_button(supabase: Supabase):
    ...
    if user["qr_status"] == QRCodeStatus.REVEALED.value:
        ...
        last_fail_user = participants.sort_values(by="penalty_time", ascending=False).dropna(
            subset=["penalty_time"]
        )
...
if supabase is None:
    ...
    revealed_user_qr_code_button(supabase)
